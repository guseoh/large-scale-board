...
동시에 이 프로젝트는 **AI가 구현을 쉽게 대신할 수 있는 환경에서도 백엔드 시스템의 동작 원리와 설계 판단을 스스로 설명하고 검증할 수 있는 역량을 기르기 위한 장기 학습 프로젝트**다. Java와 Spring 사용법만 익히는 것이 아니라, 실제 시스템을 발전시키는 과정에서 JPA·Database·OS·Network·Concurrency·Data Structure·Infrastructure·Distributed System 지식을 서로 연결하고, 왜 문제가 발생했는지와 어떤 선택이 더 적절한지를 판단하는 힘을 기르는 것을 목표로 한다.

따라서 코드를 빠르게 완성하는 것보다 **현재 구조를 이해하고, 필요한 개념을 학습하고, 설계하고, 구현하고, 측정하고, 결과를 다시 설계에 반영하는 과정**을 더 중요하게 본다. 모든 개념을 동일한 깊이로 파고들지는 않지만, 현재 시스템과 문제를 이해하는 데 필요한 개념과 백엔드 개발자가 실무에서 판단하기 위해 알아야 하는 내용은 충분한 깊이까지 학습한다.
...
* AI가 생성한 코드나 기술 제안을 그대로 수용하지 않고, 동작 원리와 시스템 맥락을 기준으로 검토하고 판단할 수 있다.
* Java, Spring, JPA, Database, OS, Network, Concurrency와 Infrastructure 지식을 실제 요청·데이터 흐름에 연결해 설명할 수 있다.
* 구현 전에 요구사항, 도메인 관계, 데이터 모델, 요청 흐름과 시스템 구조를 설계하고 설계 근거를 설명할 수 있다.
* 현재 문제를 해결하는 데 필요한 개념은 충분한 깊이까지 학습하되, 모든 주제를 동일한 깊이로 확장하지 않는 기준을 익힌다.
...
13. 구현 전에 현재 요구사항, 도메인 관계, 데이터 모델, 요청 흐름과 시스템 구조를 먼저 생각하고 설계한다.
14. Query Method, Transaction, Connection Pool, Docker Network처럼 구현 과정에 처음 등장하는 중요한 개념은 코드만 제시하지 않고 등장 이유, 핵심 원리, 현재 코드와의 연결, 주요 한계까지 학습한다.
15. 모든 개념을 동일한 깊이로 설명하지 않는다. 현재 문제와 시스템을 이해하는 데 핵심인 개념, 그리고 백엔드 개발자가 설계·운영·트러블슈팅에서 판단하기 위해 필요한 내용은 깊게 학습하고, 현재와 직접 관련 없는 내부 세부 구현은 필요할 때 별도 심화 학습으로 확장한다.
16. OS, Network, Thread, Socket, I/O, Memory, Data Structure 같은 CS 지식은 별도 암기 과목으로 분리하지 않고 실제 병목·동시성·통신·인프라 문제를 설명하는 데 필요할 때 시스템 흐름과 연결해 학습한다.
17. 인프라 작업에서도 명령어를 먼저 복사해 실행하지 않는다. 구성 요소가 필요한 이유, 요청과 연결의 흐름, 장애 모델과 운영 Trade-off를 이해한 뒤 실제 환경을 구성한다.
...
Phase 구현에 들어가기 전에 **대규모 게시판이라는 단순한 도메인을 충분히 이해하고, 이후 시스템 발전의 기준선으로 사용할 수 있을 정도의 초기 프로젝트 설계**를 먼저 만든다.

거대한 PRD나 완성형 아키텍처를 미리 만드는 것이 목적은 아니다. 초기 설계는 현재 시스템이 무엇을 제공하고 어떤 데이터와 요청 흐름을 가지는지 이해하기 위한 기준선이며, 이후 각 Phase의 실험과 문제를 통해 계속 수정된다.
...
→ 초기 데이터 모델과 ERD
→ 초기 API와 요청 흐름
→ 초기 Application / Database Architecture
→ 비기능 요구사항과 부하·장애 가설
→ 현재 Phase에서 실제 구현할 Physical Design 결정
```

초기 전체 도메인은 개념 수준에서 다음과 같은 후보를 볼 수 있다.
...
하지만 전체 도메인을 처음부터 테이블과 코드로 구현하지 않는다. **Conceptual Design은 전체 시스템의 방향과 관계를 이해하기 위해 사용하고, Physical Design과 실제 코드는 현재 Phase에 필요한 범위만 만든다.**

예를 들어 초기 설계에서 Comment, Like, View의 존재를 인지할 수는 있지만 Phase 1에서 이들을 미리 구현하지 않는다. 댓글 모델은 Phase 2에서 계층형 데이터 문제를 경험하며 구체화하고, Like는 Phase 3의 동시성 문제와 함께 설계하며, View는 Phase 4의 높은 쓰기 트래픽을 경험하면서 데이터 저장 위치와 일관성 요구사항을 다시 판단한다.

ERD와 아키텍처 역시 한 번 그리고 끝나는 산출물이 아니다.

```text
초기 ERD
→ 계층형 댓글 설계로 변경
→ 동시성 제약과 Engagement 데이터 추가
→ Outbox / Event 모델 추가
→ CQRS Read Model 분리
→ 서비스 분리 후 Database per Service로 재설계
```

각 변화에서 `무엇이 바뀌었는가`보다 **왜 기존 설계가 부족해졌고 어떤 Trade-off 때문에 새로운 설계를 선택했는가**를 학습한다.
...
* [x] Phase 0. 최소 애플리케이션 실행과 MySQL 연결 재검증
* [ ] Project Design Baseline. 프로젝트 범위·Actor/Use Case·Business Rule·Domain Map·초기 ERD·API·초기 아키텍처·부하/장애 가설 정리
