...
public class SecurityConfig {
    ...
    @Bean UserDetailsService userDetailsService(MemberRepository members) {
        return username -> members.findByUsername(username)
            .map(member -> User.withUsername(member.getUsername()).password(member.getPassword()).roles("USER").build())
    ...
    @Bean SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.csrf(csrf -> csrf.disable())
            .authorizeHttpRequests(requests -> requests.requestMatchers("/api/members").permitAll().anyRequest().authenticated())
